    ...
    if request.method == "POST":
        ...
        if user:
            if check_password_hash(user.password, password):
                flash(
                    f"Welcome to Nicko's Gallery  {user.first_name}", category="success"
                )
                login_user(user)
...
def register():
    if request.method == "POST":
        ...
        if user:
            ...
            flash(
                "The name fields should not be empty or less than two characters",
                category="error",
            )

        elif len(password) < 4:
            flash("The password should be atleast 4 characters wide", category="error")
            ...
            db.session.commit()
            flash(
                f"Welcome to Nicko's Gallery  {new_account.first_name}",
                category="success",
            )
